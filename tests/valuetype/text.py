...
def test_trailing_escape_text():
    ...
    assert TextConverter.parse("text\\,with\tdangling escape") == "text,with\tdangling escape"
    assert TextConverter.serialize(
        "text,text\\,with\tdangling escape\\") == "text\\,text\\\\\\,with\tdangling escape\\\\"
...
def test_trailing_escape_value_list():
    ...
    assert [TextConverter.parse(v) for v in TextConverter.split_value_list(cl2.value)] == \
           ["this is", "a list , with a\\", "trailing escape"]
