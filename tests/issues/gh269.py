...
def test_issue_269_missing_alarms():
    ...
    e.alarms = [
        ...
        AudioAlarm(timedelta(minutes=-5), attach=urlparse("http://example.com/alarm.ogg")),
        AudioAlarm(e.begin - timedelta(minutes=5), attach=urlparse("http://example.com/image.jpg")),
        EmailAlarm(datetime.utcfromtimestamp(0), repeat=0, duration=timedelta(minutes=0), summary="Subject",
                   description="Body",
                   attendees=[Attendee(urlparse("test@example.com"), common_name="Tester")],
                   attach=[urlparse("http://example.com/image.jpg")]),
        EmailAlarm(e.begin, repeat=5, duration=timedelta(minutes=5),
                   summary="Subject", description="Body",
                   attendees=[Attendee(urlparse("test@example.com"), common_name="Tester"),
                              Attendee(urlparse("room@example.com"), user_type="ROOM")]),
        DisplayAlarm(timedelta(minutes=5), repeat=5, duration=timedelta(minutes=5), description="ALARM!")
    ]
    e.name = "Test Event"
    ...
    for a1, a2 in zip(deser.events[0].alarms, c.events[0].alarms):
        ...
        assert not [p for p in a1.extra_params.pop("ATTENDEES", []) if p != {'__merge_next': ['FALSE']}]
        assert not [p for p in a1.extra_params.pop("ATTACH", []) if p != {'__merge_next': ['FALSE']}]
        assert a1 == a2
    assert deser == c
