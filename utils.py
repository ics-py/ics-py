...
grammar = """
contentline   = name:name (";" param:param )* ":" value crlf -> (name, param)
name          = iana_token | x_name
iana_token    = (alpha | digit | "-")+
alpha         = anything:x ?(x in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ') -> x
x_name        = "X-" (vendorid "-"){0,1} (alpha | digit | "-")+
vendorid      = (alpha | digit){3} (alpha | digit)*
...
non_ascii     = alpha
control       = ascii00_08 | ascii0a_1f | "\x7f"
crlf          = '\r\n'
